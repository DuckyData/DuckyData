...
duckyData.service('duckyDataFileUploader', function (FileUploader, toastr, $window) {
    ...
    uploaderAudio.filters.push({
        ...
        fn: function (item, options) {
            ...
            var mimeTypeSctring = '|mpeg|wav|x-wav|x-pn-wav|ogg|flac|x-ms-wma|x-ms-wmv|';


            if (chromeReg.test(browser)) {
                console.log('find chrome')
                mimeTypeSctring = '|mp3|wav|x-wav|x-pn-wav|ogg|flac|x-ms-wma|x-ms-wmv|';
            }

            if (mimeTypeSctring.indexOf(type) !== -1) {
                return true;
            } else {
                toastr.error('We only support WAVE, FLAC, OGG, MP3 and WMA', 'Unrecongnize File Type');
                return false;
            }
        }
    });
