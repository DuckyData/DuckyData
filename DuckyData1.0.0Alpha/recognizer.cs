...
namespace ACRCloud
{
    ...
    public class Program
    {
        ...
        public string go()
        {
            ...
            config.Add("access_key", "9d6f7352aa54f0384556d03f1b26aaef");
            config.Add("access_secret", "V9Ru9njotgFbOQFx1VRYS2m2lOSdnxax0sI20SZK");
            ...
            result = re.RecognizeByFileBuffer(file, file.Length, 0);
            //Console.WriteLine(result);
            ...
            var ms = Convert.ToDouble(duration);
            var s = ms / 1000.0;
            duration = Math.Round(s,0) + " seconds";

            var genre = m.Between(result, "genres", "acrid");
            var gcount = genre.Count(x => x == '[');

            var album = m.Between(result, "album name", "artists");

            var artist = m.Between(result, "artists", "timestamp_utc");
            var acount = artist.Count(x => x == '[');
